...
followers = db.Table('followers',
	...
	) # only need relationship so no need a class
...
class User(db.Model, UserMixin):
	...
	tweets = db.relationship('Tweet', backref='author', lazy='dynamic') # 'Tweet' stands for class Tweet # backref='author' User # 1 to more relationship # not a column of User
	
	followed = db.relationship(
		'User', secondary=followers,
		primaryjoin=(followers.c.follower_id==id), # followe who
		secondaryjoin=(followers.c.followed_id==id), # followed by who
...
class Tweet(db.Model):
	...
	user_id = db.Column(db.Integer, db.ForeignKey('user.id')) # user: table name # default setting: class: User, table: user

	def __repr__(self):
		return "user={}, tweet={}, create at {}".format(self.user_id, self.body, self.create_time)